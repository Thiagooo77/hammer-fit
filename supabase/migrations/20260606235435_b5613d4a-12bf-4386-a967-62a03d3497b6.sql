...
BEGIN
  ...
  -- Limpa cálculo automático anterior do mesmo dia
  DELETE FROM public.time_bank
    WHERE user_id = NEW.user_id
      AND reference_date = v_date
      AND notes LIKE '[auto]%';

  -- Saldo atual (excluindo o dia atual, pois acabamos de remover)
  SELECT COALESCE(SUM(
    CASE
      WHEN kind = 'devedora' THEN -hours
      ELSE hours
    END
  ), 0)
  ...
  -- Compensação automática quando sinais são opostos
  IF (v_current_balance > 0 AND v_diff_hours < 0) OR (v_current_balance < 0 AND v_diff_hours > 0) THEN
    ...
    -- Registra a compensação como ajuste (sinal oposto ao saldo atual)
    ...
    -- Saldo restante do dia, se houver
...
REVOKE EXECUTE ON FUNCTION public.calc_time_bank_on_punch() FROM PUBLIC, anon, authenticated;
